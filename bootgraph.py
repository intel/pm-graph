...
def printHelp():
	...
	' [submit]\n'\
	'  -submit           Submit the timeline to online DB (requires -dmesg)\n'\
...
if __name__ == '__main__':
	...
	if testrun:
		...
		# rerun with submit
		if 'submit' in db:
			db['url'] = base64.b64decode('aHR0cDovL3dvcHIuamYuaW50ZWwuY29tL2J1Z3ppbGxhL3Jlc3QuY2dp').decode()
			db['apikey'] = base64.b64decode('cTljQTRQTkJZRkVSMXFRYmdTRnpOM0VmRFM1QTlPcnN0YWVJWjc3dA==').decode()
			if 'user' not in db or 'pass' not in db:
				db['user'] = base64.b64decode('Ym9vdGdyYXBoLXRvb2w=').decode()
